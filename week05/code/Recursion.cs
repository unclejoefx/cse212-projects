...
public static class Recursion
{
    ...
    {
        // Base case: we've built a permutation of the desired size
        if (word.Length == size)
        {
            results.Add(word);
            return;
        ...
        {
            // Choose the letter at position i
            char chosenLetter = letters[i];

            // Remove this letter from available letters for the next recursion
            string remainingLetters = letters.Remove(i, 1);

            // Recurse with the chosen letter added to word and reduced letters
    ...
    public static void SolveMaze(List<string> results, Maze maze, int x = 0, int y = 0, List<ValueTuple<int, int>>? currPath = null)
    {
        ...
        // Recursive case: Try all 4 directions (right, down, left, up)
        // Right (x+1, y)
