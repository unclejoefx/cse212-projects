...
public class TakingTurnsQueueTests
{
    ...
    // Defect(s) Found: None - This test passes with finite turns. 
    ...
    // Run 10 times.
    // Expected Result: Bob, Tim, Sue, Bob, Tim, Sue, Tim, Sue, Tim, Tim
    // Defect(s) Found: People with 0 turns (infinite) are not re-enqueued because the condition
    // only checks if turns > 1. Tim should remain in the queue forever but is removed after the first turn. 
    ...
    // Defect(s) Found: People with negative turns (infinite) are not re-enqueued because the condition
    // only checks if turns > 1. Tim should remain in the queue forever but is removed after the first turn. 
